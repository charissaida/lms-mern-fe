...
const LoResultAnswer = () => {
  ...
  useEffect(() => {
    const fetchTaskAndSubmission = async () => {
      try {
        const [taskRes, submissionRes] = await Promise.all([axiosInstance.get(API_PATHS.TASKS.GET_TASK_BY_TYPE("lo")), axiosInstance.get(API_PATHS.TASKS.GET_SUBMISSION_BY_ID_USER("lo", user._id))]);
