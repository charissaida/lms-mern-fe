...
const PretestResultAnswer = () => {
  ...
  useEffect(() => {
    const fetchTaskAndSubmission = async () => {
      try {
        const [taskRes, submissionRes] = await Promise.all([axiosInstance.get(API_PATHS.TASKS.GET_TASK_BY_TYPE("pretest")), axiosInstance.get(API_PATHS.TASKS.GET_SUBMISSION_BY_ID_USER("pretest", user._id))]);
  ...
  if (loading) return <div className="text-center mt-10">Memuat hasil pretest...</div>;
...
export default PretestResultAnswer;
