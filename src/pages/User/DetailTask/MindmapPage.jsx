  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const resSub = await axiosInstance.get(API_PATHS.TASKS.GET_SUBMISSION_MINDMAP_BY_ID_USER(id, user._id));
        if (resSub?.data?.file) {
      ...
      <div className="flex flex-col md:flex-row gap-6 p-6">
        ...
        <div className="flex-1">
          ...
          <div className="bg-white p-4 rounded-lg shadow">
            ...
            <button onClick={handleSubmit} disabled={!!submission} className={`w-full mt-4 py-2.5 rounded-md text-white text-lg transition ${submission ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"}`}>
            ...
            <button
              type="button"
              disabled={!submission}
              onClick={() => window.open(`${import.meta.env.VITE_API_BASE_URL}/uploads/${submission.file}`, "_blank")}
              className={`w-full mt-3 py-2.5 rounded-md text-white text-lg transition ${submission ? "bg-gray-600 hover:bg-gray-700" : "bg-gray-300 cursor-not-allowed"}`}
            >
              Nilai
            </button>
          </div>
