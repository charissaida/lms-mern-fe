...
const MateriPage = () => {
  ...
  useEffect(() => {
    const fetchMateri = async () => {
      try {
        const res = await axiosInstance.get(API_PATHS.TASKS.GET_MATERIALS_BY_TYPE("materi"));
      ...
      <div className="flex flex-col md:flex-row gap-6 p-6">
        ...
        <div className="flex-1">
          ...
          <div className="bg-white p-4 rounded-lg shadow mb-6">
            ...
            <button onClick={() => navigate("/survey", { state: { taskTitle: "materi", taskId: materi._id } })} className="bg-blue-600 w-full cursor-pointer text-white px-6 py-2 rounded-md mt-6 hover:bg-blue-700">
              Selesai
            </button>
          </div>
        </div>
